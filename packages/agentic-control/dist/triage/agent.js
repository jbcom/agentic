...
import { z } from 'zod';
import { closeMCPClients, getMCPTools, initializeMCPClients, } from './mcp-clients.js';
...
export class Agent {
    ...
    async execute(task, options) {
        ...
        const steps = [];
        const recordStep = (toolName, input, output, approved) => {
    ...
    async *stream(task) {
        await this.initialize();
        const steps = [];
        const recordStep = (toolName, input, output) => {
...
5. Repeat until all tests pass or you've tried 5 times`;
        ...
        const testRuns = result.steps.filter((s) => s.toolName === 'bash' &&
            typeof s.input === 'object' &&
            s.input !== null &&
            'command' in s.input &&
            String(s.input.command).includes('test')).length;
        return {
            success: result.success,
            result: result.result,
            iterations: testRuns,
    ...
    async commitChanges(message) {
        const result = await this.execute(`Stage all changes and commit with message: "${message}"
...
3. Output the commit SHA`);
        ...
        const commitStep = result.steps.find((s) => s.toolName === 'bash' && s.output.includes('commit'));
        let commitSha;
        if (commitStep) {
    ...
    async buildToolSet(recordStep) {
        const requiresApproval = (toolName) => {
            return this.config.approval?.requireApproval?.includes(toolName) ?? false;
        };
        const checkApproval = async (toolName, input) => {
            if (!requiresApproval(toolName))
                return true;
            if (!this.config.approval?.onApprovalRequest)
                return true;
 ...
 */
export async function runSmartTask(task, config) {
