...
export function safeGitCommand(args: string[], options: SpawnSyncOptions = {}) {
  ...
  const allowedGitCommands = [
    'diff', 'log', 'show', 'status', 'branch', 'remote', 'config',
    'rev-parse', 'ls-remote', 'fetch', 'pull', 'push', 'clone',
    'checkout', 'merge', 'rebase', 'commit', 'add', 'reset'
  ];
...
export function safeDockerCommand(args: string[], options: SpawnSyncOptions = {}) {
  ...
  const allowedDockerCommands = [
    ...
    'start', 'stop', 'restart', 'rm', 'rmi', 'logs', 'inspect',
    'create', 'cp', 'stats', 'top', 'version', 'info'
  ];
