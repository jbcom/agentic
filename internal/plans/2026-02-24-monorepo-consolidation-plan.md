...
**Design doc:** `internal/plans/2026-02-24-monorepo-consolidation-design.md`
...
extras = docs
commands =
    sphinx-build -b markdown internal/sphinx docs/src/content/docs/api
```
...
build-sphinx:
    uv run sphinx-build -b markdown internal/sphinx docs/src/content/docs/api
...
In `packages/agentic/package.json`, change:
```json
...
```bash
git add packages/triage/ packages/agentic/package.json pnpm-lock.yaml
...
**Files:**
- Create: `python/pytest-agentic-crew/` (from crew repo's `tooling/pytest-agentic-crew/`)
...
rsync -av --exclude='__pycache__' --exclude='.pytest_cache' \
  --exclude='*.egg-info' --exclude='.ruff_cache' --exclude='.mypy_cache' \
  ~/src/agentic-dev-library/crew/tooling/pytest-agentic-crew/ python/pytest-agentic-crew/
```
...
**Files:**
- Create: `internal/sphinx/conf.py`
- Create: `internal/sphinx/index.md`
...
# Used with: sphinx-build -b markdown internal/sphinx docs/src/content/docs/api
```
...
Run: `uv run sphinx-build -b markdown internal/sphinx docs/src/content/docs/api`
Expected: Generates markdown files in docs/src/content/docs/api/

**Step 3: Commit**

```bash
git add internal/sphinx/
