...
import { EscalationStateManager } from './state.js';
/**
...
export class EscalationLadder {
    ...
    shouldSkipLevel(level, state) {
        ...
        if (level === 6) {
            if (!this.config.cloudAgentEnabled)
                return true;
            if (this.config.cloudAgentApprovalRequired && !state.approved)
                return true;
    ...
    hasCloudAgentApproval(task) {
        ...
        if (labels?.includes('approved:cloud-agent'))
            return true;
        ...
        if (Array.isArray(approved))
            return approved.includes('cloud-agent');
        return false;
    }
}
