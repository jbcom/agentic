...
describe('Production Release Properties', () => {
  ...
  describe('Example test for repository structure', () => {
    ...
    it('should have example configuration files in docs directory', async () => {
      const exampleFiles = [
        join(WORKSPACE_ROOT, 'docs/src/content/docs/examples/typescript.md'),
        join(WORKSPACE_ROOT, 'docs/src/content/docs/examples/python.md'),
        join(WORKSPACE_ROOT, 'docs/src/content/docs/examples/cli-workflows.md'),
      ];
...
describe('Property 6: Docker package installation', () => {
  ...
  it('should install both packages in Docker image', async () => {
    ...
    // /agentic is built from source (monorepo packages)
...
describe('Property 7: Multi-architecture support', () => {
  ...
  it('should configure multi-architecture builds', async () => {
    // Multi-arch builds are in release workflow (releases), not CI (PRs)
    // CI uses single-arch to avoid QEMU memory issues on runners
    const releaseWorkflow = await import('node:fs').then((fs) =>
...
describe('Documentation files example tests', () => {
  ...
  it('should have installation guide with required sections', async () => {
    ...
    expect(content).toContain('## 1. Install');
    expect(content).toContain('## 2. Initialize');
    expect(content).toContain('## 3. Set Environment Variables');
  });
  ...
  it('should have quickstart tutorial with code examples', async () => {
    ...
    expect(content).toContain('# Quick Start');
    expect(content).toContain('## 4. Spawn Your First Agent');
    expect(content).toContain('## 5. Try AI Triage');
    expect(content).toContain('```bash');
    expect(content).toContain('agentic fleet spawn');
    expect(content).toContain('agentic triage quick');
  });

  it('should have architecture documentation', async () => {
    const architecturePath = join(WORKSPACE_ROOT, 'docs/src/content/docs/guides/architecture.md');
