 ...
 */
import { normalizePriority, normalizeStatus, normalizeType, priorityToNumber, } from './types.js';
export class JiraProvider {
    ...
    async createIssue(options) {
        ...
        if (!issue)
            throw new Error(`Failed to retrieve created issue ${result.key}`);
        return issue;
    }
    async getIssue(id) {
        try {
    ...
    async updateIssue(id, options) {
        ...
        if (options.description)
            fields.description = this.stringToADF(options.description);
        if (options.priority)
            fields.priority = { name: this.mapPriorityToJira(options.priority) };
        if (options.type)
            fields.issuetype = { name: this.mapTypeToJira(options.type) };
        if (options.labels)
    ...
    async transitionIssue(id, status) {
        ...
        const transition = transitions.transitions.find((t) => t.name.toLowerCase() === jiraStatus.toLowerCase() ||
