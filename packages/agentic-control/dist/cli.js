...
handoffCmd
    ...
    .addOption(new Option('--merge-method <method>', 'Merge method')
...
program
    ...
    if (isInteractive) {
        ...
        config.fleet.autoCreatePr = autoPr;
        ...
        config.triage.provider = provider;
        // Model selection
        const modelChoice = await select({
            ...
            choices: [
                {
                    value: 'list-cursor',
                    name: 'List available Cursor models (requires CURSOR_API_KEY)' +
                        (hasCursorKey ? ' ✅' : ' ⚠️'),
                },
        ...
        if (selectedModel) {
            config.triage.model = selectedModel;
        }
        ...
        config.triage.model = hasAnthropicKey
    ...
    console.log(`   Auto-create PRs: ${config.fleet.autoCreatePr}`);
});
// ============================================
// Parse and Run
// ============================================
