...
export const safeConsole = {
  log: (message: string, ...args: unknown[]) => {
    console.log(sanitizeError(message), ...args.map(arg => 
      typeof arg === 'string' ? sanitizeError(arg) : arg
    ));
  },
  
  error: (message: string, ...args: unknown[]) => {
    console.error(sanitizeError(message), ...args.map(arg => 
      typeof arg === 'string' ? sanitizeError(arg) : arg
    ));
  },
  
  warn: (message: string, ...args: unknown[]) => {
    console.warn(sanitizeError(message), ...args.map(arg => 
      typeof arg === 'string' ? sanitizeError(arg) : arg
    ));
  },
};