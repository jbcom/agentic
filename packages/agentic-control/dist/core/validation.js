 ...
 */
export function validateEnvVar(envVar, description) {
    const value = process.env[envVar];
    if (!value || value.trim() === '') {
        const desc = description ?? envVar;
        throw new ConfigurationError(`Missing required environment variable: ${envVar}. Please set ${desc} to continue.`, ConfigErrorCode.MISSING_REQUIRED_FIELD, envVar);
    }
    return value.trim();
}
/**
 ...
 */
export function validateEnvVarWithMessage(envVar, purpose) {
    const value = process.env[envVar];
    if (!value || value.trim() === '') {
        throw new ConfigurationError(`${purpose} requires ${envVar} environment variable. Please set it and try again.`, ConfigErrorCode.MISSING_REQUIRED_FIELD, envVar);
    }
    return value.trim();
}
/**
...
export function validateRepository(repo) {
    ...
    if (!repoPattern.test(repo)) {
        throw new ConfigurationError(`Invalid repository format: ${repo}. Expected format: owner/repo`, ConfigErrorCode.INVALID_VALUE, 'repository');
    }
}
/**
...
export function validateGitRef(ref) {
    if (!/^[a-zA-Z0-9._/-]+$/.test(ref)) {
        throw new ConfigurationError(`Invalid git reference format: ${ref}`, ConfigErrorCode.INVALID_VALUE, 'gitRef');
    }
    if (ref.length > 200) {
        throw new ConfigurationError(`Git reference too long: ${ref} (max 200 characters)`, ConfigErrorCode.INVALID_VALUE, 'gitRef');
    }
}
/**
...
export function validatePositiveInt(value, field) {
    ...
    if (Number.isNaN(parsed) || parsed <= 0) {
        throw new ConfigurationError(`${field} must be a positive integer, got: ${value}`, ConfigErrorCode.INVALID_VALUE, field);
    }
    return parsed;
}
