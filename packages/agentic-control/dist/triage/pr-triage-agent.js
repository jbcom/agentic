...
${unaddressedFeedback
            .map((f, i) => `${i + 1}. [${f.severity}] ${f.source}: ${f.content}
...
4. Report what was fixed and what couldn't be auto-fixed`;
        ...
        while (iterations < maxIterations) {
            ...
            if (analysis.status === 'ready') {
                ...
                if (options.autoMerge) {
                    await this.agent.execute(`Merge PR #${prNumber} using squash merge and delete the branch`);
                }
                return {
                    success: true,
                    finalStatus: 'ready',
     ...
     */
    async waitForCI(prNumber, timeout = 300000) {
 ...
 */
export async function triagePR(repository, prNumber, options) {
...
//# sourceMappingURL=pr-triage-agent.js.map