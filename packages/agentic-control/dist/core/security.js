 ...
 */
export function createSafeError(message, originalError) {
...
export const safeConsole = {
    log: (message, ...args) => {
        console.log(sanitizeError(message), ...args.map((arg) => (typeof arg === 'string' ? sanitizeError(arg) : arg)));
    },
    error: (message, ...args) => {
        console.error(sanitizeError(message), ...args.map((arg) => (typeof arg === 'string' ? sanitizeError(arg) : arg)));
    },
    warn: (message, ...args) => {
        console.warn(sanitizeError(message), ...args.map((arg) => (typeof arg === 'string' ? sanitizeError(arg) : arg)));
    },
};
