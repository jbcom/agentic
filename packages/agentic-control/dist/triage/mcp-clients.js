 ...
 */
export async function initializeMCPClients(overrides = {}) {
    const clients = {};
    ...
    for (const [name, serverConfig] of Object.entries(mcpConfig)) {
        ...
        if (override) {
            if ('apiKey' in override && override.apiKey)
                token = override.apiKey;
            if ('token' in override && override.token)
        ...
        try {
            if (config.mode === 'stdio' && config.command) {
                ...
                if (token) {
                    ...
                    if (name === '21st-magic')
                        env.TWENTY_FIRST_API_KEY = token;
                }
